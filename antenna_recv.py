...
def print_to_screen(status_dict):
    ...
    # Print the title.
    print ' No.\tAz.\tAlt.\tAzWarn\tAltWarn\tAltLim\tOnline'
...
def parse_status(Status):
    ...
    Parse antenna status byte with following format:
        bit7: Altitude warning.
        bit6: Altitude service enabled.
        bit5: Altitude Up limited.
        bit4: Altitude Down limited.
        bit3: Azimuth warning.
        bit2: Azimuth service enabled.
    ...
    Status >>= 3         # Remove Azimuth useles bits.
    AzWarn = 'WARN' if Status % 2 == 1 else '  '     # This is bit3.
    Status >>= 1
    ...
    return AzWarn, AltWarn, AltLimit
        ...
        while not evt.isSet(): # when not terminated, do the loop.
            ...
            try:
                if (Start_flag == '$') and (End_flag == '*'): # Ensure valid data.
                    ...
                    AzWarn, AltWarn, AltLimit = parse_status(Status) # Parse status.

                    Online = 'OFF' if Online == 1 else 'ON'
                    antenna_status[SN] = Az, Alt, AzWarn, AltWarn, AltLimit, Online # Store antenna to dict. 
                #print 'size = ', que.qsize(), 'left = ', 50-que.qsize() # Check if the Queue is not enough.
            except:
                print 'Error: serial data = %s' % serial_data
                raise
